...
class Planet(db.Model, SerializerMixin):
    ...
    scientists = association_proxy('missions', 'scientist',
                                   creator=lambda scientist_obj: Mission(scientist=scientist_obj))
...
class Scientist(db.Model, SerializerMixin):
    ...
    planets = association_proxy('missions', 'planet',
                                creator=lambda planet_obj: Mission(planet=planet_obj))
    ...
    def validate_name(self, key, name):
        if not name:
            raise ValueError('Pleaes enter a name.')
        return name
...
class Mission(db.Model, SerializerMixin):
    ...
    def validate_scientist_id(self, key, scientist_id):
        if not scientist_id:
            raise ValueError('Mission must be associated with an existing scientist.')
        return scientist_id
    
    @validates('planet_id')
    def validate_planet_id(self, key, planet_id):
        if not planet_id:
            raise ValueError('Mission must be associated with an existing planet.')
        return planet_id
